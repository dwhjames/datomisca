...
object DatomicParser extends JavaTokenParsers {
  ...
  def factWithTempId: Parser[(DId, Keyword, DatomicData)] = did ~ keyword ~ (drefRestrictedKeyword | datomicData) ^^ {
    case id ~ kw ~ dd => (id, kw, dd)
  }

  def factWithFinalId: Parser[(Long, Keyword, DatomicData)] = datomicLong ~ keyword ~ (drefRestrictedKeyword | datomicData) ^^ {
    case id ~ kw ~ dd => (id.toLong, kw, dd)
  }

  def fact: Parser[(DId, Keyword, DatomicData)] = factWithTempId | (factWithFinalId ^^ { case (id, kw, dd) => (DId(id), kw, dd) })
  ...
  def add: Parser[AddFact] = brackets(addKeyword ~> fact) ^^ { case (id, kw, dd) => AddFact(id, kw, dd) }
  def addParsing: Parser[AddFactParsing] = brackets(addKeyword ~> factParsing) ^^ { fact => AddFactParsing(fact) }

  def retract: Parser[RetractFact] = brackets(retractKeyword ~> factWithFinalId) ^^ { case (id, kw, dd) => RetractFact(id, kw, dd) }
