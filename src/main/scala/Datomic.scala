...
trait DatomicTypeWrapper {
  ...
  implicit def toDWrapper[T](t: T)(implicit ddw: DatomicDataWriter[T]): DWrapper = DWrapperImpl(Datomic.toDatomic(t)(ddw))
...
trait DatomicFacilities extends DatomicTypeWrapper{
  ...
  /** Converts any value to a DatomicData given there is the right [[DDWriter]] in the scope
    ...
    * }}}
    */
  def toDatomic[T](t: T)(implicit ddw: DatomicDataWriter[T]): DatomicData = ddw.write(t)
